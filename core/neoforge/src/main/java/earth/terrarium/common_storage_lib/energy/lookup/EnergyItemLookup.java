...
public final class EnergyItemLookup implements ItemLookup<ValueStorage, ItemContext>, RegistryEventListener {
    ...
    private static final ItemCapability<ValueStorage, ItemContext> CAPABILITY = ItemCapability.create(Identifier.fromNamespaceAndPath(CommonStorageLib.MOD_ID, "energy_item"), ValueStorage.class, ItemContext.class);
    ...
    public @Nullable ValueStorage find(ItemStack stack, ItemContext context) {
        ...
        if (!stack.isEmpty()) {
            ...
            EnergyHandler handler = stack.getCapability(Capabilities.Energy.ITEM, access);
            if (handler != null) {
                IEnergyStorage storage = IEnergyStorage.of(handler);
                return new CommonItemEnergyStorage(storage, stack, context);
            }
        }
        return null;
    }
    ...
    public void register(RegisterCapabilitiesEvent event) {
        registrars.forEach(registrar -> registrar.accept((getter, items) -> {
           event.registerItem(Capabilities.Energy.ITEM, (stack, ignored) -> {
